...
#gzip preprocessed
pigz -p "$threads" "${HPATH}/${code}"/preprocess/preprocess/03-Trimming/*.fq | tee -a "${HPATH}/${code}/${code}.log"
...
#gzip/rm bac & not_bac
pigz -p "$threads" "${HPATH}/${code}"/bacterial* | tee -a "${HPATH}/${code}/${code}.log"
pigz -p "$threads" "${HPATH}/${code}"/not_bacteria* | tee -a "${HPATH}/${code}/${code}.log"
pigz -p "$threads" "${HPATH}/${code}"/DetectedBacteria.txt | tee -a "${HPATH}/${code}/${code}.log"
...
pigz -p "$threads" "${HPATH}/${code}"/intermediate_* | tee -a "${HPATH}/${code}/${code}.log"